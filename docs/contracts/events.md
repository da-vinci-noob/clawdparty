...
| type | payload | notes |
|---|---|---|
| `request_header` | `{ provider, credential_source, model, effort, system_prompt_digest, tool_schemas_digest, messages_digest?, plugins[] }` | Digests, not contents. **`messages_digest` is OPTIONAL (v1.16)** and absent on older headers — a reader MUST report those as unverified, never as matching. It is also deliberately excluded from the emit-on-change fingerprint: the messages array grows every turn, so fingerprinting it would emit a header per request. It therefore describes only the turn whose header carried it, which is why `reconstruct()` reports `not_at_boundary` for any longer prefix instead of a false mismatch. `credential_source` is a source **identity** (`CredentialSourceId`) — **never a value**. That is the whole point of the field: it makes "which login did this run use?" answerable without a credential entering the record. |
