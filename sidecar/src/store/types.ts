 ...
 * (the design record).
 *
 * `session.lock` is present here but ABSENT from the design record's list, which
 * declares itself exhaustive. The store contract's Concurrency section requires it
 * ("a `session.lock` register holding the owning process's identity plus a
 * heartbeat timestamp"), so the two documents disagree. Resolved in favour of the
 * behavioural contract, since one-writer-per-session is not optional. Recorded
