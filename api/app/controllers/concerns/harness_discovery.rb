...
# Session-scoped capability discovery proxied from the harness (connectors +
...
module HarnessDiscovery
  ...
  def fetch_discovery(kind, session)
    ...
    Rails.cache.fetch("harness/#{kind}/#{cwd}", expires_in: DISCOVERY_CACHE_TTL) do
      yield(Harness::Client.new, cwd).body
    end
  end
end
