# run-capability-selection Specification

## Purpose
TBD - created by archiving change run-tools-connectors-skills. Update Purpose after archive.
## Requirements
...
### Requirement: Capability selection shapes the run's real tool set

The harness SHALL apply the run-start payload to the tool set it offers the model, such that an OFF tool is genuinely unavailable rather than merely un-pre-approved — it owns tool dispatch, so there is no vendor option layer to map through. `disallowed_tools` SHALL remove those tools from the set the run offers, so the model never sees them — there is no pre-approval list to fall through to, because that is what made a whitelist useless as a disable. Each enabled connector name SHALL be resolved against host-owned config into an MCP server connection whose tools join the run's set, with the client never supplying a server's command/url/headers. When `skills` is non-empty (or `"all"`), the harness SHALL index the selected skills in the system prompt and offer a `skill` tool that loads one on demand — indexing, not inlining, because inlining every `SKILL.md` does not scale (79 on the measured host); when omitted/empty, no skills are indexed and no `skill` tool is offered. `cwd` SHALL remain pinned to the session worktree in all cases.

#### Scenario: An OFF tool is absent from the run's tool set
