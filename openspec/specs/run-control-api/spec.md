# run-control-api Specification

## Purpose
TBD - created by archiving change run-orchestration. Update Purpose after archive.
## Requirements
### Requirement: Role-gated run-control endpoints forward to the harness
...
SHALL forward to the harness via `Harness::Client`.
...
- **THEN** the action is permitted and forwarded to the harness via `Harness::Client`
...
### Requirement: Harness::Client is the only Rails→harness caller and is configurable

`Harness::Client` SHALL be the sole Rails→harness caller for `POST /runs`, `POST /runs/:id/messages`, and
`POST /runs/:id/interrupt`, targeting `HARNESS_URL` (default `http://harness:8787`) with no hard-coded host. It
SHALL map the frozen `harness-protocol` responses: `202` on accepted start, `200` on accepted follow-up/interrupt,
`409` when a run is already active (surfaced to the client as `409`), and `404` for an unknown run.

#### Scenario: Client targets the configurable harness URL

- **WHEN** Rails calls the harness
- **THEN** `Harness::Client` uses `HARNESS_URL` (default `http://harness:8787`), not a hard-coded address

#### Scenario: Active-run conflict surfaces as 409

- **WHEN** a start is attempted while the harness reports a run already active (`409`)
...
`POST /api/sessions/:id/runs` SHALL create the run, append `run_started`, call the harness (which returns `202`),
...
- **THEN** the endpoint responds promptly after the harness accepts (`202`), and the run's later progress arrives
