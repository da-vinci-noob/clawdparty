# harness-normalizer-full Specification

## Purpose
TBD - created by archiving change harness-runner. Update Purpose after archive.
## Requirements
...
the `harness-normalizer-v1` capability defines it.
...
A normalizer test SHALL feed the harness-owned raw fixtures (`harness/test/fixtures/raw_run.jsonl`, captured by
...
sync and the normalizer doubles as contract verification (the `harness-normalizer-v1` deferred task 5.4, now
unblocked).
...
The full mapping SHALL preserve the invariant that `normalizer.ts` is the ONLY harness file touching raw SDK
