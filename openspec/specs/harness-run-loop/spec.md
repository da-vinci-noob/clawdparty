# harness-run-loop Specification
...
run-start payload, per the frozen `event-envelope`/`harness-protocol`. Per-run `seq` SHALL be scoped to this run
...
`active_run_ids` (no longer the empty skeleton set), per the frozen `harness-protocol`.
