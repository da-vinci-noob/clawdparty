# harness-transport Specification
...
- **THEN** delivery is idempotent — Rails skips the duplicates and the harness treats the retry as successful
...
- **WHEN** a `POST /internal/events` batch receives a 401 (bad or missing `HARNESS_SHARED_SECRET`)
...
#### Scenario: Harness keeps running during a Rails outage

- **WHEN** Rails is unreachable for a period
- **THEN** the harness continues to accept and normalize events and does not crash

### Requirement: Configurable Rails base URL

The transport SHALL obtain the Rails callback base URL from configuration and SHALL NOT hard-code it, consistent with the `harness-protocol` capability's no-hard-coded-host rule. This allows pointing at a stub/log sink before `rails-foundation` lands `/internal/events`, and at the real endpoint afterward, with no code change.
