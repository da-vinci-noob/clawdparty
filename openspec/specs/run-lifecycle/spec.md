...
### Requirement: Runs::Start creates the worktree, records base_sha, and calls the harness
...
- **WHEN** the run starts
- **THEN** the `requested_by` Rails sent is what the harness stamps as `run_started.actor = { kind: "user", id }`,
...
polling. On `run_started` (the harness's first event) it SHALL transition the run `queued → running`. On a
...
The harness SHALL NOT finalize run state; Rails owns every transition. Finalization SHALL be driven from
...
- **WHEN** the harness's `run_started` event is ingested for a `queued` run
- **THEN** `Runs::Finalize` transitions the run to `running` (Rails owns the transition; the harness only emitted
  the event)
...
- **THEN** `Runs::Finalize` transitions the run to `awaiting_review` (Rails finalizes; the harness only emitted
  the event)
