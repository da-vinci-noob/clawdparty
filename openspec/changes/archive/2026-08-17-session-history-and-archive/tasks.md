...
- [X] 5.3 Manual end-to-end, run as `bin/verify-session-history` against the live stack rather than by hand: three sessions (two hosted, one joined by invite) all appear with the right `owned`/`my_role`/`status`, ordered by `last_activity_at` descending, and posting a message to the OLDEST moves it to the front — static creation order would pass an ordering check and fail that one. Archive as owner returns 200 + `status: archived`, the row's badge flips instead of the row disappearing, a second archive is a 200 no-op, a non-owner gets 403, and an unauthenticated list is 404. **It found a defect the existing 409 spec could not see:** starting a run on an archived session returned the PROVIDER's 422 ("lists no models this host can serve") because the controller resolved a model — over the network — before anything checked the session was closed. The green spec passes because model resolution succeeds in tests. Fixed by checking `archived?` first, which is the argument `validate_lane!` already makes one line above it, with two specs including one asserting no provider is consulted at all
