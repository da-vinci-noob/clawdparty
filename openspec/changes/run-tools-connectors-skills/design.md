...
### D5 — (superseded by D9) `run_started` echo removed
The original plan echoed the resolved capabilities on `run_started`. Once capabilities became always-on with no per-run selection (D9), the echo was pure noise — every run would carry the full connector + skill lists (dozens of entries) in a persisted, broadcast event, and the UI showed a giant chip. So the echo was **removed**: `run_started` carries only `{ model, cwd, permission_mode, claude_session_id }` again, and the run banner shows only the permission-mode chip. The optional `RunStartedPayload` fields remain in the contract (additive, harmless) but are unused/reserved. Original rationale kept below for history.
