...
### D4 — Skills passed as names (or `"all"`); `settingSources` side effects are bounded, not ignored
`skills` is either `"all"` or an explicit name array validated against discovered skills. When non-empty the sidecar sets `settingSources:["user","project"]` + `skills`, which auto-adds the `Skill` tool. **Default:** skills **default-OFF** (empty) so runs are unchanged unless a user opts in.
