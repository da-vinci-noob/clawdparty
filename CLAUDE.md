...
[container: harness] Node Fastify :8787 (UNPUBLISHED — compose-network only)  — sibling service, NOT a child of Rails
...
Rails reaches the harness at http://harness:8787 (compose DNS; configurable via HARNESS_URL)
...
`packages/contracts/fixtures/sample_run.jsonl` (captured from a real SDK spike) is the **executable contract**: the web renders it, a Rails fake-Claude rake task replays it through real ingest, the harness's normalizer tests assert producing it.
...
- **Two-tier streaming text:** `ai_text_delta` is **ephemeral — broadcast, never persisted** (coalesced ~150ms in the harness); `ai_text` is the durable record emitted on block stop. `presence_changed` is also ephemeral.
...
- **Bindings are deliberate:** only the `rails` container **publishes** a port (`3000` → LAN); the `harness` (`:8787`) and `vite` (`:5173`) services are **unpublished** — reachable only on the private compose network (the Docker equivalent of loopback-only). Nothing in the app may assume a fixed host: Rails reaches the harness via `HARNESS_URL` (default `http://harness:8787`), and nothing assumes `localhost` for the browser side — that is what keeps Tailscale a future drop-in (publish/forward + origins, no app changes). Auth = role-scoped reusable invite links → signed httpOnly cookie (no `Secure` flag on plain-HTTP LAN); the cookie also authenticates ActionCable.
...
- **Run the stack:** `bin/start` (= `docker compose build && docker compose up`) — brings up the `rails`, `harness`, `jobs` (Solid Queue), `postgres`, and (dev) `vite` containers. Each process is its own compose service; the harness is a sibling service with its own restart policy, **not** a child of Rails. Source is bind-mounted (`:delegated`); gems/node_modules live in named volumes; the harness read-only binds host `~/.claude` + `~/.aws` (credentials) and inherits the host's Claude/AWS auth env (so it uses the dev's existing login — API key, subscription/enterprise OAuth, or Bedrock — with no app-owned key). The **target repo is bind-mounted read-write** (Claude edits the worktree; reject reverts it) — only the credential mounts are read-only.
