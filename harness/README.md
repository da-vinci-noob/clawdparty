...
- Unpublished on the compose network as `http://harness:8787` (only `rails`
  publishes a port). Reaches Rails via `RAILS_INTERNAL_URL` (distinct from the
  Rails→harness `HARNESS_URL`).
...
The harness owns **no** Anthropic credential and selects **no** auth method. The
...
- **macOS subscription/enterprise OAuth** lives in the **Keychain with no file**,
  ...
  export `CLAUDE_CODE_OAUTH_TOKEN` so the harness inherits a usable token.
...
The five required gate suites run individually so a single gate can be exercised
...
Provider SDKs are installed ahead of the adapters that use them, so the dependency
set settles once rather than shifting per milestone. Each is scoped:
